...
def login_user():
  ...
  # if len(users) == 0: # Can use this instead, too
  if len(users) != 1: # If there are (mistakenly) two users with the same nickname in the database, this conditional will prevent a login to the first user that it finds with the given username [can use other conditional here considering that the relevant valedations are performed when a user is registered]
